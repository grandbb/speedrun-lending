...
contract Lending is Ownable {
    ...
    function _calculatePositionRatio(address user) internal view returns (uint256) {
        ...
        return (calculateCollateralValue(user) * 1 ether) / borrowed;
    }
    ...
    function liquidate(address user) public {
        ...
        if (!i_corn.transferFrom(msg.sender, address(this), debt)) revert Lending__RepayingFailed();

        s_userBorrowed[user] = 0;
        uint256 collateralToSeize = (debt * 1 ether) / i_cornDEX.currentPrice();
        uint256 reward = (collateralToSeize * LIQUIDATOR_REWARD) / 100;
        uint256 payout = collateralToSeize + reward;
        if (payout > s_userCollateral[user]) payout = s_userCollateral[user];
